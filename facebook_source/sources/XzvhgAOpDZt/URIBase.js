__d("URIBase", ["URIRFC3986", "URISchemes", "copyProperties", "ex", "invariant"]function(global, require, requireDynamic, requireLazy, module, exports, URIRFC3986, URISchemes, copyProperties, ex, invariant) {
    ...
    function n(p, q, r, s) {
        ...
        q = q.toString();
        var t = URIRFC3986.parse(q) || {};
        if (!r && !URISchemes.isAllowed(t.scheme)) return false;
        ...
        if (t.userinfo !== null)
            if (r) {
                throw new Error(ex('URI.parse: invalid URI (userinfo is not allowed in global URI): %s', p.toString()));
            } else return false;
        if (!p.getDomain() && p.getPath().indexOf('\\') !== -1)
            if (r) {
                throw new Error(ex('URI.parse: invalid URI (no domain but multiple back-slashes): %s', p.toString()));
            } else return false;
        if (!p.getProtocol() && m.test(q))
            if (r) {
                throw new Error(ex('URI.parse: invalid URI (unsafe protocol-relative URLs): %s', p.toString()));
            } else return false;
        return true;
    }
    ...
    o.prototype.setProtocol = function(p) {
        "use strict";
        invariant(URISchemes.isAllowed(p));
        this.$URIBase1 = p;
        return this;
    };
    ...
    o.prototype.setDomain = function(p) {
        "use strict";
        if (l.test(p)) throw new Error(ex('URI.setDomain: unsafe domain specified: %s for url %s', p, this.toString()));
        this.$URIBase2 = p;
        return this;
    };
    ...
    o.prototype.addQueryData = function(p, q) {
        ...
        if (p instanceof Object) {
            copyProperties(this.$URIBase6, p);
